...
# with 2 sockets per node, and 12 cores per socket, for a total of 24 cores. However, the 12 cores on the socket
# share L3 cache and main memory, so binding to cores will cause each core to stomp on eachother's cache and memory,
# and dramatically slow down, hence the -bind-to socket:2 and --ntasks-per-socket=1.
